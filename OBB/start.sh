#!/bin/bash
set -e

echo "=== Iniciando configuración ==="

export DISPLAY=:1
export XDG_RUNTIME_DIR=/tmp/runtime-$USER
mkdir -p $XDG_RUNTIME_DIR
chmod 700 $XDG_RUNTIME_DIR

# KDE SIN OPENGL (evita pantalla negra)
export KWIN_COMPOSE=QPainter
export KWIN_OPENGL_INTERFACE=software
export QT_XCB_GL_INTEGRATION=none

# DBus
echo "Configurando DBus..."
if [ ! -d /var/run/dbus ]; then
    sudo mkdir -p /var/run/dbus
    sudo dbus-daemon --system --fork
fi

# Iniciar sesión de DBus
eval $(dbus-launch --sh-syntax)
export DBUS_SESSION_BUS_ADDRESS
export DBUS_SESSION_BUS_PID

# PRIMERO: Crear el servidor X con Xvfb
echo "Iniciando servidor X virtual (Xvfb)..."
Xvfb :1 -screen 0 1280x800x24 -ac +extension GLX +render -noreset &
XVFB_PID=$!

echo "Esperando a que Xvfb esté listo..."
sleep 3

# Verificar que X está funcionando
echo "Verificando servidor X..."
if xdpyinfo -display :1 > /dev/null 2>&1; then
    echo "✓ Servidor X funcionando correctamente"
else
    echo "✗ Error: Servidor X no está funcionando"
    exit 1
fi

# SEGUNDO: Conectar x11vnc al display existente
echo "Iniciando x11vnc..."
x11vnc \
  -display :1 \
  -nopw \
  -forever \
  -shared \
  -rfbport 5901 \
  -noxdamage \
  -repeat \
  -cursor arrow \
  -bg \
  -xkb \
  -noxrecord \
  -noxfixes

echo "Esperando a que x11vnc esté listo..."
sleep 2

# TERCERO: Iniciar gestor de ventanas
echo "Iniciando gestor de ventanas..."
openbox &
WM_PID=$!

echo "Esperando a que el gestor de ventanas inicie..."
sleep 3

# CUARTO: Verificar archivos de noVNC
echo "Verificando archivos de noVNC..."
ls -la /opt/novnc/ | head -20

# QUINTO: Iniciar noVNC con la ruta correcta
echo "Iniciando noVNC en puerto 6080..."
cd /opt/novnc
/usr/bin/websockify --web=/opt/novnc 6080 localhost:5901 &
WEBSOCKIFY_PID=$!

sleep 2

echo ""
echo "=== Sistema listo ==="
echo "Accede a: http://localhost:6080/vnc.html"
echo "También puedes probar: http://localhost:6080/vnc_lite.html"
echo ""

# Función para limpiar procesos al salir
cleanup() {
    echo "Deteniendo servicios..."
    kill $WEBSOCKIFY_PID 2>/dev/null || true
    kill $WM_PID 2>/dev/null || true
    kill $DBUS_SESSION_BUS_PID 2>/dev/null || true
    kill $XVFB_PID 2>/dev/null || true
    exit 0
}

trap cleanup SIGTERM SIGINT

# Iniciar sysbank después de un momento
sleep 2
echo "Iniciando aplicación sysbank..."
/usr/local/bin/sysbank &

# Mantener el contenedor vivo
tail -f /dev/null